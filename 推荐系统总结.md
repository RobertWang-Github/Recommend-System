...
&emsp;&emsp;1.特征表示    
&emsp;&emsp;&emsp;&emsp;第i个group feature(不知道为什么用的是group而不是field)的encoding vector表示为<img src="http://latex.codecogs.com/gif.latex?t_i\inR^{K_i},t_i[j]\in\{0,1\},\sum_{j=1}^{K_i}t_i[j]=k.k=1意味着t_i是one-hot encoding,k>1是multi-hot encoding.特征表示向量x=[t_1^T,t_2^T,...,t_M^T]^T,显然t_i为列向量,\sum_{i=1}^MK_i=K,K_i为第i个group特征的encoding维数,K\text{为整个特征空间的维度}.">,  
&emsp;&emsp;2. 
&emsp;&emsp;3.  
&emsp;&emsp;4.  <img src="http://chart.googleapis.com/chart?cht=tx&chl= \text{显然}t_i\text{为列向量}" style="border:none;">
